...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is pressed, we convert the current number displayed on the screan to a number and then store the result in Calculator.First_Operand if it doesn't already exist
    ...
    if (First_Operand == null) {
        ...
        //if operator exists, property lookup is performed for the operator in the Perfom_Calculation object and the function that matches the operator is executed
...
keys.addEventListener('click', (event) => {
    ...
    const { target } = event;
    //if the element that was clicked on is not the button, exit the function
    ...
    //ensures that AC clears the cumbers from the Calculator
